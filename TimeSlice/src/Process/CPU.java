...
public class CPU {
    static int  timeSlice = (int)(Math.random()*(5-1+1)+1) ;//CPU一个时间片时长（随机生成）
    ...
    public PCB Cpu(PCB pcb){
        ...
        if(serverTime<=timeSlice){
            ...
            System.out.println("时间片（随机生成，范围1-5）:" + timeSlice + "" + "; 本次处理的进程：" + pcb.getName() +"; 进程进入CPU时刻："
            ...
            System.out.println("时间片" + timeSlice + "" + "; 本次处理的进程为：" + pcb.getName() +"; 进程进入CPU时刻："
                    + pcb.getArriveTime() + "; 进程还需运行的时间：" + serverTimeResidue );
        }                               //进程所需服务时间大于一个时间片
        return pcb;
